...
// ─── MENU RANK BY PERIOD ────────────────────────────────────────────────────
function getMenuRankByPeriod(params) {
  const dateFrom = params.dateFrom;
  const dateTo   = params.dateTo;
  if (!dateFrom || !dateTo) return { success: false, message: 'missing dateFrom/dateTo' };

  const sessions = sheetData('order_sessions').rows;
  const items    = sheetData('order_items').rows;

  const paidInRange = sessions.filter(function(s) {
    const d = String(s.created_at).substring(0, 10);
    return s.status === 'paid' && d >= dateFrom && d <= dateTo;
  });

  if (paidInRange.length === 0) return { success: true, data: [] };

  const sessionIds = {};
  paidInRange.forEach(function(s) { sessionIds[String(s.id)] = true; });

  const rangeItems = items.filter(function(i) { return sessionIds[String(i.session_id)]; });
